...
class MainActivity : android.app.Activity() {
    ...
    private fun buildUi() {
        ...
        val description = TextView(this).apply {
            text = "The app automatically creates a new scenic nature wallpaper with a Bible verse about every 8 hours. It uses a battery-friendly background schedule, stores each wallpaper below, and rotates verses and nature scenes to avoid repeats."
            textSize = 17f
            setPadding(0, 0, 0, 20)
        }
        val status = TextView(this).apply {
            ...
            text = if (last > 0L) {
                "Last wallpaper: ${dateFormat.format(Date(last))}\nVerses used before repeat: ${WallpaperHistoryStore.usedVerseCount(this@MainActivity)} of ${Verses.all.size}\nScene themes used before repeat: ${WallpaperHistoryStore.usedSceneCount(this@MainActivity)} of ${SceneStyles.all.size}"
            } else {
                "Automatic schedule becomes active after the app is opened once. No wallpaper has been generated yet."
            }
            textSize = 14f
            setPadding(0, 0, 0, 22)
        }
        ...
        val note = TextView(this).apply {
            text = "This app is optimized for low battery drain. Android may delay updates in battery saver, Doze, or heavy optimization modes, so wallpaper changes happen around every 8 hours instead of exactly on the dot."
            textSize = 13f
